...
export function NotaDialog({
  ...
  const opciones = useMemo(() => {
    if (origenFijo)
      return [{ id: origenFijo.id, etiqueta: origenFijo.etiqueta, contactos: origenFijo.contactos ?? [] }];
    if (f.ambito === "expediente")
      return expedientes.map((e) => ({ id: e.id, etiqueta: `${e.codigo} · ${e.nombre}`, contactos: [e.contactoId] }));
    if (f.ambito === "oportunidad")
      return oportunidades.map((o) => ({ id: o.id, etiqueta: `${o.codigo} · ${o.titulo}`, contactos: [o.contactoId] }));
    if (f.ambito === "persona")
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-1.5">
              ...
              >
                ...
                <SelectContent>
                  {AMBITOS.map((a) => (
                    <SelectItem key={a} value={a} disabled={a === "ejecucion" || a === "presupuesto"}>
                      {AMBITO_META[a].label}
          ...
          <div className="space-y-1.5">
            ...
            {origenFijo ? (
              <p className="rounded-md border bg-muted/40 px-3 py-2 text-sm">{origenFijo.etiqueta}</p>
            ) : (
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" className="w-full justify-between font-normal">
                  {f.contactos.length ? `${f.contactos.length} contacto(s)` : "Sin contactos concretos"}
          ...
          <div className="flex flex-wrap gap-4">
            <label className="flex items-center gap-2 text-sm">
              <Checkbox checked={f.destacada} onCheckedChange={(v) => set("destacada", Boolean(v))} />
              Destacada
            </label>
            <label className="flex items-center gap-2 text-sm">
          ...
          <Collapsible open={avanzado} onOpenChange={setAvanzado}>
            ...
            <CollapsibleContent className="mt-3 space-y-4 rounded-md border border-border p-3">
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-1.5">
                  <Label className="text-xs uppercase tracking-wide text-muted-foreground">
                    Vigencia
                  ...
                  <Select
                    ...
                    onValueChange={(v) => {
                      ...
                      setF((p) => ({
                        ...p,
                        vigencia: val,
                        vencimiento: val === "temporal" && !p.vencimiento ? sumarDias(30) : p.vencimiento,
                      }));
                    }}
                  >
                    <SelectTrigger>
                ...
                {f.vigencia === "temporal" ? (
                  <>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Select value={f.alVencer} onValueChange={(v) => set("alVencer", v as AlVencer)}>
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="archivar">Archivar automáticamente</SelectItem>
                          <SelectItem value="confirmar">Dejar pendiente de confirmación</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                  </>
                ...
                <div className="space-y-1.5">
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="restringida">Restringida a usuarios concretos</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                {f.visibilidad === "restringida" ? (
                  <div className="space-y-1.5">
                    ...
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button variant="outline" className="w-full justify-between font-normal">
                          {f.autorizados.length ? `${f.autorizados.length} usuario(s)` : "Seleccionar"}
                          <ChevronDown className="h-4 w-4 opacity-60" />
                        </Button>
                      </DropdownMenuTrigger>
