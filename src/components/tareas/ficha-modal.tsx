        ...
        <DialogContent className="flex max-h-[92vh] w-[96vw] max-w-5xl flex-col gap-0 overflow-y-auto p-0">
          <DialogHeader className="border-b border-border p-5">
            <div className="flex flex-wrap items-center gap-1.5">
              ...
              {senales.sinAbrir ? <ToneBadge tono="info">Sin abrir por el responsable</ToneBadge> : null}
          ...
          <div className="grid gap-5 p-5 lg:grid-cols-[minmax(0,1fr)_320px]">
            <div className="space-y-5">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {abierta && ejecutor && tarea.estado !== "En espera" ? (
                  <Button size="sm" variant="outline" className="gap-1.5" onClick={() => setEspera(true)}>
              ...
              {!mando ? (
                <p className="text-xs text-muted-foreground">
                  ...
                  rechazarlo motivadamente, pero su contenido y su plazo sólo los cambia quien lo encargó.
                </p>
              ) : null}

              {impedimentos.length ? (
                <div className="rounded-md border border-dashed border-warning/60 bg-warning/10 p-3 text-xs text-warning-foreground">
                  {impedimentos.map((m) => (
                    <p key={m}>{m}</p>
                  ))}
                </div>
              ...
              {tarea.esperandoRespuesta && abierta ? (
                <div className="flex flex-wrap items-center gap-2 rounded-md border border-primary/30 bg-primary/10 p-3 text-xs">
                ...
                {subtareas.length ? (
                  subtareas.map((s) => (
                    <div
                      ...
                      className="flex items-start justify-between gap-2 rounded-md px-1 py-0.5 text-sm hover:bg-muted/50"
                    >
                    ...
                    {(tarea.conversacion ?? []).map((m) => {
                      ...
                      if (m.clase === "sistema")
                        return (
                          <p
                            key={m.id}
                            className="mx-auto max-w-[85%] text-center text-[11px] text-muted-foreground"
                          >
            ...
            <aside className="space-y-5">
                  ...
                  documentos.map((d) => (
                    <div key={d.id} className="flex items-start justify-between gap-2 text-xs">
                      <span className="flex min-w-0 items-start gap-2 text-muted-foreground">
                        ...
                        <span className="min-w-0">
                          <span className="block truncate text-foreground">{d.nombre}</span>
                      ...
                      <span className="flex shrink-0 gap-1">
                        <Link
                          to="/documentos"
                          search={{ doc: d.id }}
                          className="px-1 py-0.5 text-[11px] text-primary hover:underline"
                        >
                          Abrir
                        </Link>
                        <Button
                ...
                {(tarea.evidencias ?? []).length ? (
                  (tarea.evidencias ?? []).map((e) => (
                    <p key={e.id} className="text-xs text-muted-foreground">
                      <span className="text-foreground">{e.descripcion}</span> · {e.tipo} · {e.fecha}
                    </p>
                  ))
                ) : (
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      ops.añadirEvidenciaTarea(tarea.id, { tipo: "Documento", descripcion: evidencia.trim() });
                      setEvidencia("");
                    }}
                  >
                    Añadir
              ...
              <Collapsible>
                <CollapsibleTrigger className="flex w-full items-center justify-between gap-2 text-[11px] font-medium uppercase tracking-wide text-muted-foreground hover:text-foreground">
                  <span>Histórico</span>
                ...
                <CollapsibleContent className="mt-1.5 space-y-1.5">
                  {(tarea.historico ?? []).length ? (
                    (tarea.historico ?? []).map((h) => (
                      <p key={h.id} className="flex items-start gap-2 text-xs text-muted-foreground">
                        <Clock className="mt-0.5 h-3 w-3 shrink-0" />
                        <span>
                          <span className="text-foreground">{h.accion}</span> · {h.fecha} · {h.autor} —{" "}
                          {h.detalle}
                        </span>
                      </p>
                    ))
