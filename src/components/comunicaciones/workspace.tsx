...
export function ComunicacionesWorkspace() {
  ...
  const lista = useMemo(() => {
    ...
    if (vista === "mias")
      l = l.filter((c) => !!c.cuenta && misCuentas.includes(c.cuenta));
    else
      l = l.filter((c) => !c.cuenta || CUENTAS_DESPACHO.includes(c.cuenta));
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="ml-auto flex flex-wrap gap-2">
          <NuevaTareaRapidaDialog
            contextoLabel="el despacho"
            trigger={botonNuevaTarea}
          />
          <NuevoEmailDialog
        ...
        <CardContent className="flex flex-wrap items-center gap-2 py-3">
          ...
          {periodo === "rango" ? (
            <span className="flex items-center gap-1.5">
              <SelectorFecha value={desde} onChange={setDesde} placeholder="Desde dd/mm/aaaa" className="w-44" />
              <SelectorFecha value={hasta} onChange={setHasta} placeholder="Hasta dd/mm/aaaa" className="w-44" />
            </span>
          ) : null}
