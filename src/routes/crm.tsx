...
import { BarList, FunnelChart, ToneBadge } from "@/components/crm/ui";
...
function CrmPage() {
  ...
  const conPresupuestoEnviado = oportunidades.filter((o) =>
    ["Enviado"].includes(o.presupuestoEspejo.estado) || o.fase === "contratacion" || o.fase === "ganada",
  );
  const pct = (a: number, b: number) => (b ? Math.round((a / b) * 100) : 0);

  const embudo = [
    { etapa: "Entradas registradas", total: oportunidades.length },
    { etapa: "Cualificadas", total: oportunidades.filter((o) => o.cualificacion.apta === true).length },
  ...
  return (
    <div className="mx-auto max-w-[1400px]">
      ...
      <div className="mb-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <Tarjeta label="Presupuestos pendientes" value={presupuestos.length} vista="presupuestos" tono="aviso" />
        <Tarjeta label="Sin próxima acción" value={sinAccion.length} vista="sin-accion" tono="riesgo" />
        <Tarjeta label="Estancadas (14+ días)" value={estancadas.length} vista="estancadas" tono="riesgo" />
        <Tarjeta label="Pendientes de revisión interna" value={revisionIgor.length} vista="igor" tono="aviso" />
        <Tarjeta label="En validación de Igor" value={enValidacion.length} vista="validacion" tono="aviso" />
        <Tarjeta label="Aceptados del periodo" value={ganadas.length} vista="ganadas" tono="exito" />
        <Tarjeta label="Cerrados / perdidos" value={cerradas.length} vista="cerradas" tono="neutro" />
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <Card>
          ...
          <CardContent className="grid gap-4 sm:grid-cols-2">
            <div>
              <p className="text-xs text-muted-foreground">Conversión comercial</p>
              <p className="font-serif text-2xl font-semibold">{pct(ganadas.length, oportunidades.length)} %</p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground">Conversión desde primera cita</p>
              <p className="font-serif text-2xl font-semibold">{pct(ganadas.length, conCita.length)} %</p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground">Conversión de presupuestos</p>
              <p className="font-serif text-2xl font-semibold">
                {pct(ganadas.length, conPresupuestoEnviado.length)} %
              </p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground">Descartes internos</p>
              <p className="font-serif text-2xl font-semibold">
                {cerradas.filter((o) => o.cierre?.tipo === "Descartada por el despacho").length}
              </p>
            </div>
            <div className="sm:col-span-2">
              <p className="mb-2 text-xs text-muted-foreground">Tiempo medio por fase (días)</p>
              ...
              {activas
                ...
                .map((o) => (
                  <li key={o.id} className="flex flex-wrap items-center justify-between gap-2 py-2">
                    ...
                    <span className="text-xs text-destructive">{alertasDe(o).join(" · ")}</span>
                  </li>
                ))}
            </ul>
            <p className="mt-3 text-xs text-muted-foreground">
              {tareas.filter((t) => t.estado !== "Completada" && t.estado !== "Cancelada").length} tareas
              abiertas en el despacho.
            </p>
          </CardContent>
        </Card>
      </div>
