 ...
 */
export function integrarLectura(sesion: SesionIA, documentoId: string, lectura: Lectura): SesionIA {
  ...
  const conDocs: SesionIA = {
    ...
    documentos: sesion.documentos.map((d) =>
      d.id === documentoId ? { ...d, estado: 'leido', tipoDocumental: lectura.tipoDocumental } : d,
    ),
    datos,
    personas,
    preguntas,
