...
const VISTAS_BASE: VistaGuardada[] = [
  { id: "v-mios", nombre: "Mis expedientes", descripcion: "Expedientes en los que soy responsable", filtros: { responsable: "Marta Solé" } },
  { id: "v-espera", nombre: "En espera de tercero", descripcion: "El avance no depende del despacho", filtros: { estadoOperativo: "En espera" } },
  { id: "v-nosotros", nombre: "Debemos actuar nosotros", descripcion: "Trabajo pendiente en el despacho", filtros: { dependencia: "Debemos actuar nosotros" } },
  { id: "v-ejecucion", nombre: "En ejecución", descripcion: "Expedientes con dimensión de ejecución activa", filtros: { fase: "cumplimiento" } },
  { id: "v-riesgo", nombre: "Con alertas", descripcion: "Expedientes con alguna alerta activa", filtros: { alertas: "si" } },
];
...
function semilla(): OpsState {
  return migrarSemillaOps({
    ...
    secuencias: { MS: 100, NT: 100, CAD: 100, LT: 100, EJ: 100, AC: 100, DOC: 100, TR: 100, FC: 100, CM: 100, IN: 100, AU: 100, EX: 200, AP: 0, ET: 100, HT: 100 },
  });
}
...
function leerAlmacen(): OpsState {
  ...
  try {
    ...
    const titulos = (parsed.titulosTarea?.length ? parsed.titulosTarea : [...TITULOS_TAREA_INICIALES]).filter(
      (t) => !/reuni[oó]n/i.test(t),
    );
...
function migrarLineas(s: OpsState): OpsState {
  ...
  const lineas = s.lineas.map((l) => {
    ...
    if (estado === l.estado && l.situacion && l.prioridad && l.objetivo && typeof l.orden === "number") return l;
    cambios += 1;
...
function sembrarComunicaciones(s: OpsState): OpsState {
  ...
  const tareasNuevas = (TAREAS_COMUNICACIONES_DEMO as TareaOp[]).filter(
    (t) => !idsTareas.has(t.id),
  );
...
function migrarSemillaOps(s: OpsState): OpsState {
  return sembrarComunicaciones(unificarTareas(
    sembrarEtiquetasTareas(
      migrarEtiquetas(migrarTareas(migrarLineasFlexibles(migrarLineas(migrarActuaciones(migrarCodigos(s)))))),
    ),
  ));
}
...
function migrarLineasFlexibles(s: OpsState): OpsState {
  ...
  const lineas = s.lineas.map((l) => {
    ...
    return {
      ...l,
      responsableHeredado:
        l.responsableHeredado ?? Boolean(exp && exp.responsable && exp.responsable === l.responsable),
...
export function alertasDeLinea(s: OpsState, l: LineaTrabajo): AlertaLinea[] {
  ...
  const tareasAbiertas = v.tareas.filter((t) => t.estado !== "Completada" && t.estado !== "Cancelada");
  ...
  if (vencidas.length)
    av.push({ id: "tarea-vencida", tono: "riesgo", texto: `Existe ${vencidas.length === 1 ? "una tarea vencida" : `${vencidas.length} tareas vencidas`}.` });
  if (plazoProximo)
    av.push({ id: "plazo", tono: "aviso", texto: `Plazo próximo: ${plazoProximo.titulo} (${plazoProximo.fecha}).` });
  if (abierta && recordatorioVencido)
    av.push({ id: "recordatorio", tono: "aviso", texto: `Recordatorio vencido: ${recordatorioVencido.texto}` });
  ...
  if (abierta && l.fechaObjetivo && (diasHasta(l.fechaObjetivo) ?? 1) < 0)
    av.push({ id: "objetivo", tono: "riesgo", texto: `Fecha objetivo superada (${l.fechaObjetivo}).` });
  if (abierta && l.bloqueo) av.push({ id: "bloqueo", tono: "riesgo", texto: `Bloqueada: ${l.bloqueo}` });
  if (abierta && l.situacion === "Depende de tercero" && l.fechaSeguimiento && (diasHasta(l.fechaSeguimiento) ?? 1) < 0)
    av.push({ id: "tercero", tono: "aviso", texto: "Pendiente de respuesta de tercero; fecha de seguimiento vencida." });
  if (l.estado === "Resuelta" && (tareasAbiertas.length || v.fechas.some((f) => (diasHasta(f.fecha) ?? -1) >= 0)))
    av.push({
...
export const senalesTarea = (s: OpsState, t: TareaOp): SenalesTarea => {
  ...
  return {
    ...
    emailsEnviados: s.comunicaciones.filter((c) => c.tareaId === t.id && c.estadoEnvio === "Enviado")
      .length,
...
export const etiquetaPorId = (s: OpsState, id: string) => (s.etiquetas ?? []).find((e) => e.id === id);
...
export const selComunicaciones = (s: OpsState, expedienteId: string) =>
  s.comunicaciones.filter(
    (c) =>
      c.expedienteId === expedienteId ||
      (c.expedientesRelacionados ?? []).includes(expedienteId),
  );
...
export function alertasDeExpediente(s: OpsState, e: ExpedienteOp): Alerta[] {
  ...
  selDocumentos(s, e.id)
    .filter((d) => d.judicial && d.datosJudiciales?.estadoPlazo === "Posible plazo pendiente de validar")
    .forEach((d) => push(`Plazo sin validar: ${d.nombre}`, "riesgo", "Documento", d.id, "doc-plazo-sin-validar", 0));

  // Documentos recibidos sin clasificar
  selDocumentos(s, e.id)
    .filter((d) => esRecibido(d) && d.estado === "Sin clasificar")
    .forEach((d) => push(`Documento recibido sin clasificar`, "aviso", "Documento", d.id, "doc-sin-clasificar", 2));

  // Documentos judiciales presentados sin justificante
  selDocumentos(s, e.id)
    .filter((d) => d.judicial && d.datosJudiciales?.fechaPresentacion && !d.datosJudiciales.justificante)
    .forEach((d) =>
      push(`Presentación sin justificante: ${d.nombre}`, "aviso", "Documento", d.id, "doc-sin-justificante", 2),
    );

  // Fechas críticas sin validar / vencidas
  selFechas(s, e.id).forEach((f) => {
    if (!f.validada) push(`Fecha sin validar: ${f.titulo}`, "riesgo", "Fecha", f.id, "fecha-sin-validar", 1);
    const dh = diasHasta(f.fecha);
  ...
  selTareas(s, e.id)
    ...
    .forEach((t) => {
      ...
      if (dh !== null && dh < 0) push(`Tarea vencida: ${t.titulo}`, "riesgo", "Tarea", t.id, "tarea-vencida", 0);
    });

  // Actuaciones pendientes de reportar al cliente
  selActuacionesValidas(s, e.id)
    .filter((x) => x.estado === "Pendiente de reportar" || (x.visibleCliente && !x.clienteInformado && x.estado === "Completada"))
    .forEach((x) =>
      push(`Pendiente de reportar al cliente: ${x.titulo}`, "aviso", "Actuación", x.id, "act-sin-reportar", 2),
    );
  ...
  if (!ultimaActuacion || (dias !== null && dias > 15))
    push("Sin actuaciones registradas en más de 15 días", "aviso", "Expediente", e.id, "sin-actividad", 3);
  ...
  selEjecuciones(s, e.id).forEach((ej) => {
    if (!ej.proximoControl)
      push(`Ejecución sin próximo control: ${ej.titulo}`, "aviso", "Ejecución", ej.id, "eje-sin-control", 2);
    if (ej.situacionPresupuestaria === "No incluida" || ej.situacionPresupuestaria === "Requiere nuevo presupuesto")
      push("Ejecución fuera de presupuesto", "riesgo", "Ejecución", ej.id, "eje-fuera-presupuesto", 1);
  });

  // Documentos entregables sin versión definitiva presentados
  selDocumentos(s, e.id)
    .filter((d) => d.entregable && (d.estado === "Presentado" || d.estado === "Entregado") && !tieneDefinitiva(d))
    .forEach((d) =>
      push(`Entregable sin versión definitiva: ${d.nombre}`, "aviso", "Documento", d.id, "doc-sin-definitiva", 2),
    );
...
export function agruparAlertas(alertas: Alerta[]): AlertaAgrupada[] {
  ...
  return Array.from(mapa.values())
    .map((g) => ({
      ...g,
      texto: g.total > 1 ? (ENUNCIADO_GRUPO[g.clave]?.(g.total) ?? `${g.texto} (+${g.total - 1})`) : g.texto,
    }))
...
export const ops = {
  ...
  crearExpediente(data: Partial<ExpedienteOp> & { nombre: string; contactoId: string; naturaleza: Naturaleza }) {
    const id = nuevoId("EX");
    ...
    auditar({ accion: "Alta de expediente", entidad: "Expediente", entidadId: id, expedienteId: id, anterior: "—", nuevo: e.nombre });
    return e;
  },

  actualizarExpediente(id: string, cambios: Partial<ExpedienteOp>) {
    mapExp(id, (e) => ({ ...e, ...cambios, ultimoMovimiento: hoyTexto() }));
    auditar({ accion: "Edición de expediente", entidad: "Expediente", entidadId: id, expedienteId: id, anterior: "—", nuevo: Object.keys(cambios).join(", ") });
  },
  ...
  moverFase(id: string, fase: string) {
    ...
    if (fase === "finalizado") {
      const pendientes = [
        ...selTareas(estado, id).filter((t) => t.estado !== "Completada" && t.estado !== "Cancelada").map((t) => `Tarea: ${t.titulo}`),
        ...selFechas(estado, id).filter((f) => !f.resultado && (diasHasta(f.fecha) ?? 0) >= 0).map((f) => `Fecha: ${f.titulo}`),
        ...selEjecuciones(estado, id).filter((e) => e.estado !== "Cumplido" && e.estado !== "Liquidación y cierre").map((e) => `Ejecución: ${e.titulo}`),
      ];
      if (pendientes.length) return { ok: false as const, motivo: "Existen elementos abiertos", pendientes };
    }
    const anterior = exp.fase;
    const enCierre = fase === "cierre-encargo" || fase === "cierre-procedimiento" || fase === "liquidacion";
    mapExp(id, (e) => {
      const { suspension, ...resto } = e;
      const siguiente: ExpedienteOp = {
        ...resto,
        fase,
        estadoGeneral: fase === "finalizado" ? "Cerrado" : fase === "aparcado" ? "Aparcado" : enCierre ? "Cierre pendiente" : "Activo",
    ...
    auditar({ accion: "Cambio de fase", entidad: "Expediente", entidadId: id, expedienteId: id, anterior, nuevo: fase });
  ...
  actualizarDondeEstamos(id: string, texto: string) {
    ...
    auditar({ accion: "Cambio de «Dónde estamos»", entidad: "Expediente", entidadId: id, expedienteId: id, anterior: "—", nuevo: texto.slice(0, 120) });
  },
  ...
  fijarProximaAccionPrincipal(id: string, opciones: { actuacionId?: string; texto?: string }) {
    ...
    auditar({ accion: "Cambio de próxima acción principal", entidad: "Expediente", entidadId: id, expedienteId: id, anterior: "—", nuevo: texto || "Sin definir" });
  },
  ...
  cambiarEstadoGeneral(id: string, estadoGeneral: EstadoGeneral) {
    ...
    auditar({ accion: "Cambio de estado general", entidad: "Expediente", entidadId: id, expedienteId: id, anterior: "—", nuevo: estadoGeneral });
  },
  ...
  crearLinea(data: Omit<LineaTrabajo, "id">) {
    ...
    auditar({ accion: "Alta de línea de trabajo", entidad: "Línea", entidadId: id, expedienteId: data.expedienteId, anterior: "—", nuevo: data.nombre });
    return id;
  },

  /** Edición con trazabilidad campo a campo (usuario, fecha, valor anterior y nuevo). */
  actualizarLinea(id: string, cambios: Partial<LineaTrabajo>, accion = "Edición de línea de trabajo") {
    ...
    const tocados = trazables.filter((k) => k in cambios && String(cambios[k] ?? "") !== String(prev[k] ?? ""));
    if (!tocados.length) {
      auditar({ accion, entidad: "Línea", entidadId: id, expedienteId: prev.expedienteId, anterior: "—", nuevo: prev.nombre });
      return;
    }
  ...
  cambiarEstadoLinea(id: string, nuevoEstado: EstadoLinea, motivo?: string) {
    ...
    if (nuevoEstado !== "Cerrada" && nuevoEstado !== "Descartada") extra.fechaCierre = undefined as never;
  ...
  reordenarLineas(expedienteId: string, ids: string[]) {
    set((s) => ({
      ...s,
      lineas: s.lineas.map((l) =>
        l.expedienteId === expedienteId && ids.includes(l.id) ? { ...l, orden: ids.indexOf(l.id) + 1 } : l,
      ),
    }));
    auditar({ accion: "Reordenación de líneas", entidad: "Línea", entidadId: "—", expedienteId, anterior: "—", nuevo: `${ids.length} líneas` });
  },

  /** Sube o baja una línea una posición dentro de su expediente. */
  moverLinea(expedienteId: string, id: string, direccion: -1 | 1) {
    const orden = selLineas(estado, expedienteId).filter((l) => !l.parentId).map((l) => l.id);
  ...
  registrarAvance(id: string, texto: string, fecha = hoyTexto()) {
    ops.actualizarLinea(id, { ultimoAvance: texto, fechaUltimoAvance: fecha, dondeEstamos: texto }, "Actualización del último avance");
  },
  ...
  tareaDesdeRecordatorio(id: string) {
    ...
    const tareaId = ops.crearTarea({
      titulo: r.texto,
      descripcion: linea ? `Recordatorio de la línea «${linea.nombre}».` : "Recordatorio convertido en tarea.",
      expedienteId: r.expedienteId,
  ...
  vincularElemento(
    ...
    const clave = (
      { actuacion: "actuaciones", tarea: "tareas", documento: "documentos", fecha: "fechas", comunicacion: "comunicaciones" } as const
    )[tipo];
    let expedienteId: string | undefined;
    let etiqueta = elementoId;
    set((s) => ({
      ...s,
      [clave]: (s[clave] as { id: string; expedienteId?: string; lineaId?: string; lineasRelacionadas?: string[]; titulo?: string; nombre?: string; asunto?: string }[]).map(
        (el) => {
  ...
  desvincularRelacionada(
    ...
    set((s) => ({
      ...s,
      [clave]: (s[clave] as { id: string; lineaId?: string; lineasRelacionadas?: string[] }[]).map((el) =>
        el.id === elementoId
          ? {
              ...el,
  ...
  activarEjecucion(data: Omit<Ejecucion, "id" | "lineaId"> & { lineaId?: string }) {
    const lineaId =
      ...
      ops.crearLinea({
        ...
        nombre: data.modalidad === "Ejecución judicial" ? "Ejecución judicial" : "Cumplimiento y ejecución",
        tipo: "Ejecución",
    ...
    set((s) => ({
      ...
      expedientes: s.expedientes.map((e) =>
        e.id === data.expedienteId ? { ...e, fase: "cumplimiento", ultimoMovimiento: hoyTexto() } : e,
      ),
    }));
    auditar({ accion: "Activación de ejecución", entidad: "Ejecución", entidadId: id, expedienteId: data.expedienteId, anterior: "—", nuevo: data.titulo });
    return id;
  },

  actualizarEjecucion(id: string, cambios: Partial<Ejecucion>) {
    const prev = estado.ejecuciones.find((e) => e.id === id);
    set((s) => ({ ...s, ejecuciones: s.ejecuciones.map((e) => (e.id === id ? { ...e, ...cambios } : e)) }));
    if (prev)
      auditar({ accion: "Edición de ejecución", entidad: "Ejecución", entidadId: id, expedienteId: prev.expedienteId, anterior: prev.estado, nuevo: cambios.estado ?? prev.estado });
  },
  ...
  crearActuacion(data: Omit<Actuacion, "id">) {
    ...
    auditar({ accion: "Alta de actuación", entidad: "Actuación", entidadId: id, expedienteId: data.expedienteId, anterior: "—", nuevo: data.titulo });
    return id;
  },

  actualizarActuacion(id: string, cambios: Partial<Actuacion>) {
    set((s) => ({ ...s, actuaciones: s.actuaciones.map((a) => (a.id === id ? { ...a, ...cambios } : a)) }));
  },
  ...
  marcarComoActuacion(id: string, datos: Partial<Actuacion> = {}) {
    ...
    auditar({ accion: "Marcada como actuación", entidad: "Actuación", entidadId: id, expedienteId: a.expedienteId, anterior: "Actividad", nuevo: "Actuación" });
  },
  ...
  desmarcarActuacion(id: string) {
    ...
    auditar({ accion: "Deja de destacarse como actuación", entidad: "Actuación", entidadId: id, expedienteId: a.expedienteId, anterior: "Actuación", nuevo: "Actividad" });
  },

  /** Marca una actuación como hito histórico. Exige que sea actuación. */
  marcarHito(id: string, datos: { tituloHito: string; categoriaHito?: Actuacion["categoriaHito"] }) {
    const a = estado.actuaciones.find((x) => x.id === id);
    if (!a) return { ok: false as const, motivo: "Actividad no encontrada" };
    if (!a.esActuacion) return { ok: false as const, motivo: "Solo una actuación puede marcarse como hito." };
    ops.actualizarActuacion(id, {
      esHito: true,
    ...
    auditar({ accion: "Marcada como hito histórico", entidad: "Actuación", entidadId: id, expedienteId: a.expedienteId, anterior: "Actuación", nuevo: datos.tituloHito || a.titulo });
    return { ok: true as const };
  },
  ...
  desmarcarHito(id: string) {
    const a = estado.actuaciones.find((x) => x.id === id);
    if (!a) return;
    ops.actualizarActuacion(id, { esHito: false, modificadoPor: estado.usuario, fechaModificacion: ahora() });
    auditar({ accion: "Retirada de los hitos", entidad: "Actuación", entidadId: id, expedienteId: a.expedienteId, anterior: "Hito", nuevo: "Actuación" });
  },
  ...
  anularActuacion(id: string, motivo: string) {
    ...
    auditar({ accion: "Anulación de actividad", entidad: "Actuación", entidadId: id, expedienteId: a.expedienteId, anterior: a.estadoRegistro ?? "Confirmada", nuevo: `Anulada — ${motivo}` });
  },

  confirmarActuacion(id: string) {
    const a = estado.actuaciones.find((x) => x.id === id);
    if (!a) return;
    ops.actualizarActuacion(id, { estadoRegistro: "Confirmada", modificadoPor: estado.usuario, fechaModificacion: ahora() });
    auditar({ accion: "Confirmación de actividad", entidad: "Actuación", entidadId: id, expedienteId: a.expedienteId, anterior: a.estadoRegistro ?? "Borrador", nuevo: "Confirmada" });
  },
  ...
  cambiarEstadoActuacion(id: string, estadoNuevo: EstadoActuacion) {
    ...
    auditar({ accion: "Cambio de estado de actuación", entidad: "Actuación", entidadId: id, expedienteId: a.expedienteId, anterior: a.estado, nuevo: estadoNuevo });
  },
  ...
  actualizarDocumento(id: string, cambios: Partial<Documento>) {
    set((s) => ({ ...s, documentos: s.documentos.map((d) => (d.id === id ? { ...d, ...cambios } : d)) }));
  },
  ...
  cambiarEstadoDocumento(id: string, estadoNuevo: string) {
    ...
    if (
      ...
      return { ok: false as const, motivo: "Antes de presentar o entregar debe existir una versión definitiva." };
  ...
  nuevaVersion(id: string, version: Omit<VersionDocumento, "numero">) {
    ...
    if (tieneDefinitiva(d) && !version.definitiva)
      return { ok: false as const, motivo: "El documento tiene versión definitiva: crea un documento sustituto." };
    const numero = d.versiones.length + 1;
    ops.actualizarDocumento(id, { versiones: [...d.versiones, { ...version, numero }], version: numero });
    auditar({
  ...
  asignarDocumento(id: string, expedienteId: string, lineaId?: string) {
    ...
    auditar({ accion: "Asignación de documento", entidad: "Documento", entidadId: id, expedienteId, anterior: "Sin asignar", nuevo: expedienteId });
  },

  validarPlazoDocumento(id: string, fecha: { titulo: string; fecha: string; responsable: string; criticidad: FechaCritica["criticidad"] }) {
    const d = estado.documentos.find((x) => x.id === id);
  ...
  crearTarea(data: Omit<TareaOp, "id">) {
    ...
    const tarea: TareaOp = {
      ...
      historico: [
        { id: nuevoId("HT"), fecha: ahora(), autor: estado.usuario, accion: "Creación", detalle: data.titulo },
      ],
      ...data,
  ...
  actualizarEtiqueta(id: string, cambios: Partial<Pick<EtiquetaTarea, "nombre" | "color" | "descripcion">>) {
    if (!puedeGestionarEtiquetas(estado))
  ...
  fusionarEtiquetas(origenId: string, destinoId: string) {
    ...
    if (origenId === destinoId) return { ok: false as const, error: "Elige dos etiquetas distintas." };
    ...
    set((s) => ({
      ...
      tareas: s.tareas.map((t) => {
        ...
        afectadas += 1;
        return { ...t, etiquetas: [...new Set((t.etiquetas ?? []).map((x) => (x === origenId ? destinoId : x)))] };
      }),
    }));
  ...
  aplicarEtiquetasTarea(tareaId: string, ids: string[]) {
    ...
    if (añadidas.length)
      ops.registrarHistoricoTarea(
        tareaId,
        "Etiquetas añadidas",
        añadidas.map(nombre).join(", "),
      );
    if (quitadas.length)
      ops.registrarHistoricoTarea(
        tareaId,
        "Etiquetas retiradas",
        quitadas.map(nombre).join(", "),
      );
    return { ok: true as const };
  },
  ...
  notificar(usuario: string, texto: string, tipo: Notificacion["tipo"], tareaId?: string, fechaId?: string) {
  ...
  registrarHistoricoTarea(id: string, accion: string, detalle: string, anterior?: string, nuevo?: string) {
    const entrada = {
      id: nuevoId("HT"),
      fecha: ahora(),
      autor: estado.usuario,
  ...
  editarEncargo(
    id: string,
    cambios: Partial<Pick<TareaOp, "titulo" | "descripcion" | "vencimiento" | "horaLimite" | "prioridad" | "expedienteId" | "lineaId">>,
  ) {
    const t = estado.tareas.find((x) => x.id === id);
    if (!t) return { ok: false as const, error: "La tarea no existe." };
    if (!puedeEditarEncargo(estado, t))
      return { ok: false as const, error: "Sólo quien encargó la tarea puede modificar su contenido." };
  ...
  ampliarPlazoTarea(id: string, vencimiento: string, horaLimite: string, motivo = "") {
    ...
    ops.notificar(t.responsable, `Nuevo plazo de «${t.titulo}»: ${vencimiento} ${horaLimite}.`, "Cambio", id);
    return { ok: true as const };
  },
  ...
  cambiarEstadoTarea(id: string, estadoNuevo: EstadoTareaOp) {
    ...
    ops.registrarHistoricoTarea(id, "Cambio de estado", `${t.estado} → ${estadoNuevo}`, t.estado, estadoNuevo);
    auditar({
  ...
  completarTarea(id: string, resultado?: string) {
    ...
    ops.registrarHistoricoTarea(id, "Cierre", resultado || "Tarea completada.", t.estado, "Completada");
    auditar({
      accion: "Cambio de estado de tarea",
      entidad: "Tarea",
      entidadId: id,
  ...
  revisarDocumentosDeTarea(id: string) {
    ...
    for (const docId of t.documentosVinculados ?? []) {
      ...
      const vivas = estado.tareas.filter(
        (x) => (x.documentosVinculados ?? []).includes(docId) && ESTADOS_TAREA_VIVOS.includes(x.estado),
      );
      if (!vivas.length && d.estado === "En tratamiento") ops.actualizarDocumento(docId, { estado: "Tratado" });
    }
  },
  ...
  activarSiguienteFase(id: string) {
    ...
    const modo = sig.plazoModo ?? (sig.diasTrasPredecesora ? "dias" : sig.vencimiento ? "fija" : "sin");
    const vencimiento =
      modo === "dias" ? sumarDiasTexto(sig.diasTrasPredecesora ?? 7) : modo === "fija" ? sig.vencimiento : "";
  ...
  reactivarTarea(id: string) {
    ...
    ops.registrarHistoricoTarea(id, "Reactivación", "La tarea vuelve al trabajo vivo.", t.estado, "En curso");
  },
  ...
  marcarSubtareaConvertida(id: string, subId: string, nuevaId: string) {
    ...
    ops.actualizarTarea(id, {
      subtareas: (t.subtareas ?? []).map((x) => (x.id === subId ? { ...x, convertidaEn: nuevaId } : x)),
    });
  ...
  capturarEnInbox(titulo: string, mensaje = "") {
    ...
    const orden =
      estado.tareas.filter((t) => t.inboxDe === estado.usuario && t.etapaInbox === "Bandeja de entrada")
        .length + 1;
    ops.actualizarTarea(id, {
      capturada: true,
  ...
  moverEtapaInbox(id: string, etapa: EtapaInbox) {
    ...
    if (!t) return;
    const orden =
      estado.tareas.filter((x) => x.inboxDe === (t.inboxDe ?? estado.usuario) && x.etapaInbox === etapa)
        .length + 1;
    ops.actualizarTarea(id, {
      etapaInbox: etapa,
  ...
  completarYMarcarActuacion(id: string, resultado?: string) {
    ...
    if (!t) return { ok: false as const, motivos: ["La tarea no existe."] };
    if (!t.expedienteId)
      return { ok: false as const, motivos: ["Sólo las tareas de un expediente generan actuación."] };
  ...
  cancelarTarea(id: string, motivo: string) {
    ...
    if (t?.esSiguienteAccion)
      ops.quitarSiguienteAccion(id, "Cancelada: el contexto queda SIN SIGUIENTE ACCIÓN.");
    ops.actualizarTarea(id, { estado: "Cancelada", motivoCierre: motivo, esperandoRespuesta: undefined });
  ...
  rechazarTarea(id: string, motivo: MotivoRechazo, explicacion: string) {
    ...
    ops.notificar(t.creador ?? "", `${estado.usuario} ha rechazado «${t.titulo}»: ${motivo}.`, "Rechazo", id);
    return { ok: true as const };
  },
  ...
  gestionarRechazo(
    ...
    if (!t) return { ok: false as const, error: "La tarea no existe." };
    if (!puedeEditarEncargo(estado, t))
      return { ok: false as const, error: "Sólo quien encargó la tarea puede gestionar el rechazo." };
    ...
    if (accion === "reenviar" || accion === "mantener") {
      ...
      ops.registrarHistoricoTarea(id, "Gestión del rechazo", datos.mensaje || "Se mantiene el encargo.");
    ...
    if (accion === "saltar") {
      ...
      cerrar("Fase saltada");
      ops.actualizarTarea(id, { estado: "Cancelada", motivoCierre: `Fase saltada: ${datos.motivo}` });
  ...
  añadirEvidenciaTarea(id: string, datos: { tipo: EvidenciaTarea["tipo"]; descripcion: string; refId?: string }) {
    const t = estado.tareas.find((x) => x.id === id);
    if (!t) return;
  ...
  crearRecordatorioTarea(
    ...
    ops.registrarHistoricoTarea(tareaId, "Recordatorio", `${datos.fecha} ${datos.hora} · ${responsable}`);
    ops.notificar(responsable, `Recordatorio creado para «${t.titulo}» el ${datos.fecha} ${datos.hora}.`, "Recordatorio", tareaId, id);
    return { ok: true as const, id };
  },
  ...
  duplicarTarea(id: string) {
    const t = estado.tareas.find((x) => x.id === id);
    if (!t) return;
    const { id: _omit, historico: _h, reclamaciones: _r, evidencias: _e, conversacion: _c, rechazo: _rz, cadenaId: _cd, ordenCadena: _oc, bloqueadaPor: _bp, desbloquea: _db, ...resto } = t;
  ...
  añadirSiguienteTarea(
    ...
    ops.registrarHistoricoTarea(ultima.id, "Cadena", `Se añade la fase ${orden}: «${datos.titulo}».`);
  ...
  moverFaseCadena(id: string, direccion: -1 | 1) {
    ...
    const j = i + direccion;
    if (i < 0 || j < 0 || j >= cadena.length) return { ok: false as const, error: "Movimiento no posible." };
    const bloqueada = (x: TareaOp) => x.estado === "Pendiente" && Boolean(x.bloqueadaPor);
    if (!bloqueada(cadena[i]!) || !bloqueada(cadena[j]!))
      return { ok: false as const, error: "Sólo pueden reordenarse fases futuras todavía no activadas." };
    const orden = [...cadena];
    orden[i] = cadena[j]!;
    orden[j] = cadena[i]!;
  ...
  añadirRecordatorioARegistro(padreId: string, datos: { fecha: string; hora?: string; titulo?: string }) {
  ...
  posponerRegistro(id: string, minutos: number, destino?: { fecha: string; hora: string }) {
    ...
    if (f.origen?.tipo === "Tarea")
      ops.registrarHistoricoTarea(f.origen.id, "Recordatorio pospuesto", `${aplazamiento.anterior} → ${aplazamiento.nuevo}`);
  },
  ...
  registrarComunicacion(datos: {
    ...
    const id = ops.crearComunicacion({
      tipo: datos.canal === "Llamada" ? "Llamada" : datos.canal === "WhatsApp" ? "Mensaje" : "Email",
      canal: datos.canal,
   ...
   */
  guardarAdjuntoEnDocumentos(
    id: string,
    nombre: string,
    destino?: { expedienteId?: string },
  ) {
    const c = estado.comunicaciones.find((x) => x.id === id);
  ...
  crearTareaReunion(datos: {
    ...
    const reunion: DatosReunion = {
      ...
      ...(datos.preferenciaFechaValor ? { preferenciaFechaValor: datos.preferenciaFechaValor } : {}),
    ...
    ops.actualizarTarea(tareaId, { plantilla: "Tarea especial · Reunión", especial: { tipo: "Reunión" }, reunion });
  ...
  añadirAsistenteReunion(
    ...
    if (a.clase !== "Externo")
      ops.notificar(a.nombre.trim(), `Te han invitado a una reunión: «${t.titulo}».`, "Encargo", tareaId);
  },
  ...
  fijarParticipantesReunion(
    tareaId: string,
    participantes: { nombre: string; clase: AsistenteReunion["clase"]; rol?: string; contactoId?: string }[],
  ) {
    const t = estado.tareas.find((x) => x.id === tareaId);
    if (!t?.reunion) return;
  ...
  guardarBorradorEmail(
    ...
    if (!puedeRedactarEmail(estado, t))
      return { ok: false as const, error: "No tienes permiso para enviar comunicaciones de esta tarea." };
  ...
  registrarEnvioEmail(
    ...
    set((s) => ({
      ...s,
      comunicaciones: s.comunicaciones.map((x) => (x.id === comunicacionId ? { ...x, ...cambios } : x)),
    }));
    if (!c.tareaId) return;
    if (resultado.ok) {
      ops.registrarHistoricoTarea(
