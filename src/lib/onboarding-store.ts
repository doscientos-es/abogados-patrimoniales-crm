...
const migrarPresupuesto = (p: Presupuesto, indice: number): Onboarding | null => {
  ...
  return {
    ...
    fechaProforma,
    ...(fase === 'pago' || fase === 'completado'
      ? { fechaPago: p.proforma?.fecha ?? fechaProforma }
      : {}),
...
const registrar = (o: Onboarding, tipo: string, descripcion: string): Onboarding => ({
  ...o,
  historial: [...o.historial, { fecha: hoyTexto(), usuario: estado.usuario, tipo, descripcion }],
})
...
export const onboarding = {
  ...
  registrarProformaEnviada(datos: {
    ...
    const existente = datos.leadId
      ...
      : undefined
    if (existente)
      return { ok: false, motivo: 'Este Lead ya tiene un Onboarding.', id: existente.id }
  ...
  completar(
    ...
    if (o.expedienteId) {
      return {
        ...
        ...(o.expedienteCodigo ? { expedienteCodigo: o.expedienteCodigo } : {}),
      }
    }
    ...
    const exp = ops.crearExpediente({
      ...
      fase: 'diagnostico',
      dondeEstamos:
        o.inicioFormal?.resumen || 'Expediente abierto tras el inicio formal con el cliente.',
      ...
      registrar(
        {
          ...
          ...(datos.excepcion?.trim()
            ? { revisionMigracion: `Completado con excepción: ${datos.excepcion}` }
            : {}),
        },
        'Expediente',
