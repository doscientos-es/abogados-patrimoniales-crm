...
import { type FechaCritica, type RegistroTemporal, type ClasePlazo } from '@/data/expedientes-model'
...
export const tonoSituacion: Record<
  SituacionTemporal,
  'neutro' | 'info' | 'exito' | 'aviso' | 'riesgo'
> = {
  Pendiente: 'info',
  Realizado: 'exito',
  Pospuesto: 'aviso',
...
export function urlGoogleCalendar(f: FechaCritica) {
  ...
  const params = new URLSearchParams({
    ...
    details: [
      f.observaciones,
      f.expedienteId ? `Expediente ${f.expedienteId}` : '',
      `LEX · ${registroDe(f)}`,
    ]
      .filter(Boolean)
      .join('\n'),
