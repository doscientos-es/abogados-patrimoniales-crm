...
export function construirContextoIA(
  ...
  bloques.push(
    [
      ...
      e.suspension
        ? `Suspendido desde ${e.suspension.fecha}: ${e.suspension.motivo}`
        : "",
      e.procedimiento
  ...
  if (lineas.length) {
    bloques.push(
      "LÍNEAS DE TRABAJO\n" +
        lineas
          .map((l) => `- ${l.nombre} (${l.tipo}) · estado ${l.estado} · ${l.dondeEstamos || "sin síntesis"}`)
          .join("\n"),
    );
    lineas.forEach((l) =>
  ...
  if (ultimas.length) {
    ...
    ultimas.forEach((a) =>
      fuentes.push({ tipo: "Actuación", id: a.id, label: `${a.fecha} · ${a.titulo}`, pestana: "actuaciones" }),
    );
  }
  ...
  if (abiertas.length) {
    bloques.push(
      "TAREAS ABIERTAS\n" +
        abiertas
          .map((t) => `- [Tarea · ${t.titulo}] responsable ${t.responsable}, vence ${t.vencimiento}, estado ${t.estado}`)
          .join("\n"),
    );
    abiertas.forEach((t) =>
  ...
  if (fechas.length) {
    ...
    fechas.forEach((f) =>
      fuentes.push({ tipo: "Plazo", id: f.id, label: `${f.fecha} · ${f.titulo}`, pestana: "fechas" }),
    );
  }
  ...
  if (comunicaciones.length) {
    bloques.push(
      "COMUNICACIONES\n" +
        comunicaciones
          .slice(0, 10)
          .map((c) => `- [Comunicación · ${c.fecha}] ${c.tipo} con ${c.destinatarios.join(", ") || "—"}: ${c.asunto}`)
          .join("\n"),
    );
    comunicaciones.slice(0, 10).forEach((c) =>
      fuentes.push({ tipo: "Comunicación", id: c.id, label: `${c.fecha} · ${c.asunto}`, pestana: "comunicaciones" }),
    );
  }
  ...
  if (ejecuciones.length || e.saldoPendiente || e.presupuestoId) {
    bloques.push(
      "INFORMACIÓN ECONÓMICA\n" +
        [
          ...
          typeof e.saldoPendiente === "number" ? `- Saldo pendiente del cliente: ${e.saldoPendiente} €` : "",
          ...ejecuciones.map(
            (ej) =>
    ...
    if (ejecuciones.length || typeof e.saldoPendiente === "number") {
      fuentes.push({ tipo: "Económico", id: e.id, label: "Situación económica", pestana: "economico" });
    }
  }
  ...
  if (notas.length) {
    bloques.push(
      "NOTAS INTERNAS VINCULADAS AL EXPEDIENTE\n" +
        notas
          .map((n) => `- [Nota · ${n.creada}] ${n.titulo ? `${n.titulo}: ` : ""}${n.contenido}${n.critica ? " (nota crítica)" : ""}`)
          .join("\n"),
    );
    notas.forEach((n) =>
      fuentes.push({ tipo: "Nota", id: n.id, label: `${n.creada} · ${n.titulo || "Nota interna"}`, pestana: "notas" }),
    );
  }

  const suficiente =
    actuaciones.length + documentos.length + lineas.length + fechas.length + comunicaciones.length > 0 ||
    Boolean(e.dondeEstamos);
  ...
  return {
    ...
    huella: huellaDe([
      ...
      actuaciones[0] ? `${actuaciones[0].id}|${actuaciones[0].fecha}|${actuaciones[0].hora ?? ""}|${actuaciones[0].estado}` : "sin-actuaciones",
