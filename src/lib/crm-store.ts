  ...
  return [
    ...
    baseOportunidad({
      ...
      presupuestoEspejo: {
        ...
        historial: [
          { fecha: sumarDias(-10), usuario: 'Marta Solé', texto: 'Solicitud de presupuesto' },
        ],
      },
      ...
      presupuestoEspejo: {
        ...
        historial: [
          { fecha: sumarDias(-7), usuario: 'Luis Ferrán', texto: 'Solicitud de presupuesto' },
          {
            ...
            texto: 'Validado y enviado al cliente',
          },
        ],
        validadoVersion: 1,
    ...
    baseOportunidad({
      ...
      presupuestoEspejo: {
        ...
        historial: [
          ...
          { fecha: sumarDias(-15), usuario: 'Igor Belmonte', texto: 'Validado y enviado' },
          {
            ...
            texto: 'Devuelto: requiere modificación',
          },
        ],
        validadoVersion: 1,
        validadoPor: 'Igor Belmonte',
      ...
      contratacion: {
        ...contratacionVacia(),
        negociacion:
          'Solicita fraccionar honorarios y ampliar el alcance a la escritura de novación.',
    ...
    baseOportunidad({
      ...
      presupuestoEspejo: {
        ...
        version: 1,
        historial: [
          { fecha: sumarDias(-20), usuario: 'Igor Belmonte', texto: 'Validado por Igor' },
        ],
        validadoVersion: 1,
        validadoPor: 'Igor Belmonte',
    ...
    baseOportunidad({
      ...
      cualificacion: cual(
        true,
        'Sin conflicto y con documentación completa.',
        'Apta para primera cita',
      ),
      ...
      presupuestoEspejo: {
        ...
        version: 1,
        historial: [
          { fecha: sumarDias(-13), usuario: 'Igor Belmonte', texto: 'Validado y enviado' },
        ],
      },
      contratacion: {
        ...contratacionVacia(),
    ...
    baseOportunidad({
      ...
      presupuestoEspejo: {
        ...
        version: 1,
        historial: [
          { fecha: sumarDias(-9), usuario: 'Igor Belmonte', texto: 'Validado y enviado' },
        ],
      },
      contratacion: {
        ...contratacionVacia(),
...
function hist(
  ...
  tipo: string,
  descripcion: string,
  extra?: Partial<{ resultado: string; proxima: string }>,
) {
  return {
    ...o,
    historial: [
...
export const crm = {
  ...
  moverFase(
    id: string,
    destino: FaseId,
    opciones?: { motivo?: string; subestado?: string; forzar?: boolean },
  ): ResultadoMovimiento {
    const o = estado.oportunidades.find((x) => x.id === id)
    if (!o) return { ok: false, faltantes: [] }
  ...
  setProximaAccion(id: string, accion: ProximaAccion | null, crearTarea = false) {
    mapOp(id, (o) =>
      hist(
        ...
        'Próxima acción',
        accion
          ? `${accion.descripcion} (${accion.responsable}, ${accion.fechaPrevista})`
  ...
  guardarCita(id: string, patch: Partial<OportunidadCRM['citaCRM']>, subestado?: string) {
    ...
    mapOp(id, (op) => {
      ...
      return hist(
        actualizado,
        'Primera cita',
        patch.resultado ? `Resultado: ${patch.resultado}` : 'Datos de la cita actualizados',
      )
    })
    if (o)
      nuevaActividad({
  ...
  solicitarPresupuesto(
    ...
    const numero =
      ...
      `PR-2026-${String(4000 + estado.oportunidades.length).slice(-4)}`
    mapOp(id, (op) =>
      hist(
        {
          ...op,
          ...
          presupuestoEspejo: {
            ...
            historial: [
              ...op.presupuestoEspejo.historial,
              {
                fecha: hoyTexto(),
                usuario: estado.usuario,
                texto: `Solicitud enviada a ${datos.responsable}`,
              },
            ],
          },
        },
  ...
  cambiarEstadoPresupuesto(id: string, nuevo: EstadoPresupuestoEspejo, incidencia?: string) {
      ...
      hist(
        {
          ...
          presupuestoEspejo: {
            ...
            incidencia:
              incidencia ??
              (nuevo === 'Bloqueado' || nuevo === 'Requiere modificación'
                ? op.presupuestoEspejo.incidencia
                : ''),
    ...
    if (nuevo === 'Enviado') {
      ...
      crm.setProximaAccion(id, {
        descripcion:
          `Primer seguimiento del presupuesto ${o.presupuestoEspejo.numero || ''}`.trim(),
        tipo: 'Seguimiento',
  ...
  enviarAValidacion(id: string, observaciones?: string): ResultadoMovimiento {
    ...
    const r = crm.moverFase(id, 'validacion', {
      subestado: 'Pendiente de validación',
      forzar: true,
    })
    crm.setProximaAccion(id, {
          ...
          presupuestoEspejo: {
            ...
            historial: [
              ...op.presupuestoEspejo.historial,
              {
                fecha: hoyTexto(),
                usuario: estado.usuario,
                texto: `Devuelto para rectificación: ${instrucciones}`,
              },
            ],
          },
        },
  ...
  registrarEnvioPresupuesto(
    ...
    if (o.presupuestoEspejo.validadoVersion !== o.presupuestoEspejo.version)
      return {
        ok: false,
        faltantes: [{ label: 'La versión vigente no está validada por Igor', ok: false }],
      }
    mapOp(id, (op) =>
      hist(
        {
  ...
  registrarAceptacion(id: string, datos: Omit<AceptacionLead, 'usuario'>): ResultadoMovimiento {
    ...
    if (!datos.version)
      return {
        ok: false,
        faltantes: [{ label: 'Debe identificarse la versión aceptada', ok: false }],
      }
    mapOp(id, (op) =>
      hist(
        {
  ...
  actualizarContratacion(id: string, patch: Partial<OportunidadCRM['contratacion']>) {
    ...
    mapOp(id, (op) => {
      ...
      const checklist = {
        ...
        hoja:
          contratacion.hojaEncargo === 'Firmada' ||
          contratacion.decision === 'Aceptado verbalmente',
  ...
  toggleChecklist(id: string, clave: string, valor: boolean) {
    mapOp(id, (o) => {
      ...
      return hist(
        ...
        `${clave}: ${valor ? 'cumplido' : 'pendiente'}`,
      )
    })
  },
  ...
  convertirEnExpediente(id: string): {
    ok: boolean
    faltantes: Requisito[]
    expedienteId?: string
  } {
    ...
    if (!o) return { ok: false, faltantes: [] }
    if (o.expedienteId)
      return { ok: false, faltantes: [{ label: 'Ya existe un expediente vinculado', ok: false }] }
