...
const expediente = (overrides = {}) => ({
  id: 'case-1', referencia: 'AP_11', contactoPrincipalId: 'contact-1', oportunidadId: null,
  titulo: 'Compraventa', area: '', tipoAsunto: '', naturaleza: 'Extrajudicial',
  estadoGeneral: 'active', fase: 'intake', estadoOperativo: 'pending', prioridad: 'Media',
  asignadoId: 'user-1', fechaApertura: '2026-01-01', fechaCierre: null, proximaAccion: '',
  dondeEstamos: '', version: 1, actualizadoEn: '2026-01-01T10:00:00Z', ...overrides,
}) as never
...
describe('case control workflow', () => {
  it('normalizes legacy and operational phases into control board columns', () => {
    ...
    expect(caseControlColumn(expediente({ fase: 'Preparación y primeras actuaciones' }))).toBe('preparation')
  ...
  it('labels dependencies and reports stale, unvalidated, and overdue case work', () => {
    ...
    const alerts = caseAlerts(expediente(), [
      { expedienteId: 'case-1', tipo: 'Plazo', validacion: 'Propuesto', estado: 'Pendiente', titulo: 'Contestación', venceEn: '2026-09-06T09:00:00Z' },
    ] as never, [], now)
