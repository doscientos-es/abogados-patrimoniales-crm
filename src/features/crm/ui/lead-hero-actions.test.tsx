...
describe("LeadHeroActions", () => {
  ...
  it("selects the requested detail tab with the correct accessibility state", () => {
    ...
    expect(screen.getByRole("tab", { name: "Enviado al cliente" })).toBeTruthy();
    expect(screen.getByRole("tab", { name: "Contratación" })).toBeTruthy();
  ...
  it("presents accessible qualification questions and suggested additions", () => {
    ...
    render(
      <QualificationQuestions
        ...
        onUpdateResponse={() => undefined}
        onUpdateObservation={() => undefined}
    ...
    expect(
      screen.getByText("Todavía no hay preguntas de cualificación en este Lead."),
    ).toBeTruthy();
    expect(
  ...
  it("edits and exposes a persisted response and observation for each question", () => {
    const onUpdateResponse = vi.fn();
    const onUpdateObservation = vi.fn();
    render(
      <QualificationQuestions
        questions={[
          {
    ...
    fireEvent.change(screen.getByLabelText("Respuesta de la pregunta 1"), {
      target: { value: "Sí" },
    });
    fireEvent.change(screen.getByLabelText("Observación de la pregunta 1"), {
      target: { value: "El plazo vence esta semana." },
    });

    expect(onUpdateResponse).toHaveBeenCalledWith("q-1", "Sí");
    expect(onUpdateObservation).toHaveBeenCalledWith("q-1", "El plazo vence esta semana.");
  });
