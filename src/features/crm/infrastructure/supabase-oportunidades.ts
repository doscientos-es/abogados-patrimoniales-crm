...
const updateDetailsInputSchema = z.object({
  ...
  transicionContratacion: z
    .object({
      ...
      subestado: z.string().trim().min(1).max(160),
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<ComunicacionOportunidad[]> => {
      ...
      return (eventsResult.data ?? [])
        ...
        .filter(
          (communication): communication is ComunicacionOportunidad => communication !== null,
        );
    },
  });
}
...
export function useActualizarDetallesOportunidad(firmId: string | undefined) {
  ...
  return useMutation({
    mutationFn: async (rawInput: ActualizarDetallesOportunidadInput) => {
      ...
      const { data, error } = await client.rpc("crm_save_opportunity_details", {
        target_opportunity_id: input.id,
        target_expected_version: input.versionEsperada,
        new_details: asJson(input.detalles),
        transition_stage: input.transicionContratacion?.fase ?? null,
        transition_substage: input.transicionContratacion?.subestado ?? null,
        transition_reason: input.transicionContratacion?.motivo ?? null,
      });
      if (error?.code === "40001")
        throw new Error("Otro usuario ha modificado este Lead. Recarga la ficha antes de guardar.");
      if (error) throw error;
      const saved = oportunidadFromRow(data);
      return saved;
    },
    onSuccess: (oportunidad) => {
      queryClient.setQueryData(["crm", "oportunidades", firmId, oportunidad.id], oportunidad);
