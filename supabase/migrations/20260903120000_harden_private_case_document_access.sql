-- Document changes are only possible through the scoped RPCs below.  The former
-- permissive policy combined with the document-specific SELECT policy through OR.
drop policy if exists crm_case_documents_member on public.crm_case_documents;
drop policy if exists crm_case_documents_read on public.crm_case_documents;

create policy crm_case_documents_read on public.crm_case_documents
...
declare
  ...
  created_document public.crm_case_documents;
begin
